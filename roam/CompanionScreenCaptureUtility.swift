...
enum CompanionScreenCaptureUtility {
    ...
    /// Captures a single rectangular region of one screen at high resolution.
    /// `globalRect` is in global AppKit coordinates (bottom-left origin), as
    /// produced by the spatial "circle" gesture. The region is captured at up to 2x
    /// scale so small silkscreen text and fine traces stay legible to the model.
    static func captureRegion(globalRect: CGRect, screen: NSScreen) async throws -> CompanionScreenCapture {
        let content = try await SCShareableContent.excludingDesktopWindows(false, onScreenWindowsOnly: true)
        ...
        guard let screenNumber = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID,
            ...
            throw NSError(domain: "CompanionScreenCapture", code: -3,
                          userInfo: [NSLocalizedDescriptionKey: "Couldn't match the selected region to a display"])
        }

        // Exclude our own overlays/panels from the capture.
        let ownBundleIdentifier = Bundle.main.bundleIdentifier
        let ownAppWindows = content.windows.filter { $0.owningApplication?.bundleIdentifier == ownBundleIdentifier }

        // Convert the global AppKit rect (bottom-left origin) into a display-local
        // rect in points with a top-left origin, which is what sourceRect expects.
        let localX = globalRect.origin.x - screen.frame.origin.x
        let appKitTopEdge = globalRect.maxY - screen.frame.origin.y
        let topLeftY = screen.frame.height - appKitTopEdge
        let sourceRect = CGRect(x: localX, y: topLeftY, width: globalRect.width, height: globalRect.height)

        let filter = SCContentFilter(display: display, excludingWindows: ownAppWindows)
        let configuration = SCStreamConfiguration()
        configuration.sourceRect = sourceRect

        // Render at up to 2x the selected size (capped) for detail.
        let scale: CGFloat = 2.0
        let maxSide: CGFloat = 2048
        var outWidth = globalRect.width * scale
        var outHeight = globalRect.height * scale
        let largest = max(outWidth, outHeight)
        if largest > maxSide {
            let k = maxSide / largest
            outWidth *= k
            outHeight *= k
        }
        configuration.width = max(1, Int(outWidth))
        configuration.height = max(1, Int(outHeight))
        ...
        return CompanionScreenCapture(
            imageData: jpegData,
            label: "a region the user drew a box around (this is the focus — describe what's inside it)",
            isCursorScreen: true,
