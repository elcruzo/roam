...
final class OpenAIAudioTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    // The OpenAI key lives in the worker, not the app — so this provider is
    // always "configured" as long as the worker URL is set.
...
private final class OpenAIAudioTranscriptionSession: BuddyStreamingTranscriptionSession {
    ...
    private static let transcriptionURL = URL(string: RoamConfig.transcribeURL)!
    ...
    private func requestTranscription(for wavAudioData: Data) async throws -> String {
        ...
        // No Authorization header — the worker holds the OpenAI key and adds it.
    ...
    deinit {
        // Do NOT dispatch to stateQueue here — capturing self while the object is
        // being deallocated resurrects it and crashes (SIGABRT, "deallocated with
        // non-zero retain count"). Tear down the upload/session synchronously;
        // the in-flight state no longer matters once we're being freed.
        transcriptionUploadTask?.cancel()
        urlSession.invalidateAndCancel()
    }
}
