...
struct BlueCursorView: View {
    ...
    private let fullWelcomeMessage = "hey! i'm roam"
        ...
        ZStack {
            ...
            if buddyNavigationMode == .pointingAtTarget && !navigationBubbleText.isEmpty {
                Text(navigationBubbleText)
                    ...
                    // Wrap long answers instead of growing into one off-screen line.
                    .frame(maxWidth: min(320, max(160, screenFrame.width - 48)), alignment: .leading)
                    ...
                    // Center next to the cursor, but clamp so the bubble never
                    // runs off the left or right edge of the screen.
                    .position(
                        x: min(
                            max(cursorPosition.x + 10 + navigationBubbleSize.width / 2,
                                navigationBubbleSize.width / 2 + 8),
                            max(navigationBubbleSize.width / 2 + 8,
                                screenFrame.width - navigationBubbleSize.width / 2 - 8)
                        ),
